module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsList = newsTitles.map(title => {
      const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5) // 新聞分類底下的:新聞、公告、活動、賽事
      // test.push(...randomCats.slice(0,1))
      return {
        categories: randomCats.slice(0, 2),
        title: title
      }
    })
    // console.log('randomCats:',test.slice((a,b)=>a._id>b._id))
