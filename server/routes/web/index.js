module.exports = (app) => {
  ...
  router.get("/news/init", async (req, res) => {
    ...
    const newsTitles = [
      ...
      "破浪对决中的战船是……戳开解锁吴地艨艟的“身世之谜”！",
      "红方高地惊现新建筑？一起探秘魏都代表建筑“高阙”！",
      "王者荣耀×名创优品惊喜联名来袭！",
      "QQ飞车手游公测2周年开启，登录就送永久劳斯莱斯！",
      "7月14日全服不停机更新公告",
      "7月13日体验服停机更新公告",
      "7月10日体验服停机更新公告",
      "夏日主题荣耀播报票选活动开启公告",
    ...
    const newsList = newsTitles.map((title) => {
      const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5) // 隨機排序新聞分類底下的:新聞、公告、活動、賽事  ex:新聞、活動、賽事、公告
      return {
        categories: randomCats.slice(0, 2), // 取前兩項分類 ex:新聞、活動
        title: title,
      };
    });
  ...
  router.get("/heroes/init", async (req, res) => {
    await Hero.deleteMany({}); // 清空所有英雄數據
    const rawData = [
      {
        name: "热门",
        heroes: [
